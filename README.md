...
Not one more design opinion to install â€” the skill that knows the good ones already
...
<sub>28 commands Â· 17 reference chapters Â· 8 playbooks Â· 7 analysis scripts Â· 13 agents Â· zero dependencies</sub>

[See it work](#see-it-work) Â· [Why](#why-one-skill-instead-of-five) Â· [Install](#install) Â· [Use](#use) Â· [Commands](#the-28-commands) Â· [CLI](#cli-tools) Â· [Compatibility](#compatibility) Â· [Credits](#credits)
...
Two real rebuilds. Same content, same stack, same developer â€” the difference is the
skill running the design decisions.

<div align="center">

<img src="https://raw.githubusercontent.com/ShadowFull12/overhaul-ui/main/assets/derived/cardix-before-after-loop.webp" alt="Cardix â€” before and after, side by side" width="900">

<sub><b>Cardix</b> â€” left: before. right: after, rebuilt with overhaul-ui.
<a href="https://github.com/ShadowFull12/overhaul-ui/blob/main/assets/derived/cardix-before-after.mp4">Full 53-second walkthrough â†’</a></sub>
...
<tr>
<td colspan="2" align="center"><sub><b>Cardix</b> â€” untinted greys and default spacing â†’ one tinted OKLCH ramp, real type hierarchy, designed empty states</sub></td>
</tr>
<tr>
...
<tr>
<td colspan="2" align="center"><sub><b>Portfolio</b> â€” the template look â†’ a named direction, committed to end-to-end</sub></td>
</tr>
</table>
...
> `prefers-reduced-motion` handled, and all four async states designed. Not eyeballed â€”
...
instinct â€” and it creates a new problem. They don't know about each other. Two of
...
machine and routes to the specialist that's actually best at the task in front of it â€”
...
- Purpleâ†’blue hero gradient
- `transition: all 300ms ease-in-out`
- Command palette animates open
- Happy path only
- `outline: none`
- "Unlock the power of seamlessâ€¦"
...
```bash
# recommended â€” three directories, covers ~13 agents
npx overhaul-ui install
...
**Manual** â€” clone and copy the payload:
```bash
git clone https://github.com/ShadowFull12/overhaul-ui
cp -r overhaul-ui/skills/overhaul-ui ~/.agents/skills/
```

**Pin a version** â€” the skill payload is content, so pinning keeps behaviour stable:
```bash
...
```
"this landing page looks AI-generated, fix it"      â†’ slop
"redesign the dashboard"                            â†’ overhaul
"audit this page"                                   â†’ audit
"the animations feel janky"                          â†’ motion-audit
"add dark mode"                                      â†’ colorize
"make it accessible"                                 â†’ harden
"build a pricing page"                               â†’ shape
"is this design any good?"                           â†’ critique
```
...
1. **Reads the stack** â€” framework, styling, Tailwind v3 vs v4, existing tokens, component library
...
6. **Reports honestly** â€” what was verified, and what was not
...
| | |
|---|---|
| Visible `:focus-visible` at â‰¥3:1 on every control | Body text â‰¥4.5:1 in **both** themes |
| Touch targets â‰¥44Ã—44px | Keyboard reach and order match visual order |
| `prefers-reduced-motion` handled, not deleted | Transitions name their properties â€” never `all` |
| No `ease-in` enters, nothing over 300ms for UI | All four async states, not just the happy path |
| No layout shift from hover, focus or load | Tinted neutrals â€” no `#000`/`#fff`/`#888` |
...
**Assess**
`audit` Â· full defect list
`critique` Â· design judgment
`slop` Â· de-slop the aesthetic
`motion-audit` Â· review motion
`review` Â· pre-delivery gate
`doctor` Â· environment check
`learn` Â· explain a concept

</td><td width="33%">

**Build**
`init` Â· set up the system
`shape` Â· new interface
`components` Â· one component
`states` Â· loading/empty/error
`animate` Â· add motion
`visualize` Â· charts, dashboards
`tokens` Â· token architecture

</td><td width="33%">

**Change**
`overhaul` Â· redesign in place
`polish` Â· micro-detail pass
`typeset` Â· typography
`colorize` Â· colour, dark mode
`layout` Â· spacing, composition
`responsive` Â· sizes and inputs
`distill` Â· remove clutter
`bolder` / `quieter` Â· dial it
`copy` Â· rewrite the text
`harden` Â· WCAG 2.2 AA
`optimize` Â· Core Web Vitals
`inspire` Â· find a direction
`handoff` Â· write it up
...
`philosophy` Â· `anti-slop` Â· `direction` (18 named looks) Â· `typography` (18 pairings) Â·
`color` (OKLCH, dark mode) Â· `layout` Â· `responsive` Â· `motion` Â· `interaction-states` Â·
`components` Â· `accessibility` (WCAG 2.2) Â· `performance` (LCP/INP/CLS) Â· `design-tokens` Â·
`copywriting` Â· `data-viz` (25 chart types) Â· `stacks` Â· `libraries`

### 8 playbooks

`landing-page` Â· `saas-dashboard` Â· `portfolio` Â· `ecommerce` Â· `docs-site` Â·
`mobile-app` Â· `auth-and-forms` Â· `ai-chat-app`

### 5 templates

`tokens.css` Â· `motion.css` Â· `DESIGN.md` Â· `AUDIT.md` Â· `HANDOFF.md`
...
*compute* things instead â€” real OKLCH colour math, real WCAG contrast ratios, real
...
```bash
# AI-slop and craft defects â€” 35 rules across colour, motion, layout, a11y, copy
...
# WCAG contrast â€” a pair, or a whole token file
...
#   â†’ 13 tinted neutral steps + 11 accent steps, contrast-checked against both themes
...
```
overhaul-ui slop-scan  128 files Â· src

src/components/Hero.tsx
  error   14  Purple-to-blue gradient - the most recognisable AI-design signature
              className="bg-gradient-to-r from-violet-500 to-blue-600 â€¦"
              â†’ One flat brand colour. If depth is needed, vary lightness within one hue.
...
src/styles/global.css
  error    8  Focus outline removed with no visible replacement (SC 2.4.7)
              â†’ :focus-visible { outline: 2px solid var(--ring); outline-offset: 2px; }

slop-scan: 4 error Â· 11 warn Â· 6 info
by category: colour 7 Â· motion 6 Â· accessibility 4 Â· copy 3 Â· layout 1
```
</details>
...
Every HSL-generated palette has a muddy middle â€” this is the fix, and it's real math, not a preset.</sub>
</div>
...
|---|---|:-:|
| **Codex, Cursor, Zed, Copilot, OpenCode, Roo, Gemini CLI, Amp, Devin** | `~/.agents/skills/` | âœ… |
| **Claude Code** | `~/.claude/skills/` | âœ… |
| **Kiro** | `~/.kiro/skills/` | âœ… |
...
React Â· Next.js Â· Vue Â· Nuxt Â· Svelte Â· SvelteKit Â· Astro Â· Remix Â· React Native Â·
Expo Â· Tailwind v3 and v4 Â· CSS Modules Â· vanilla CSS Â· Vanilla Extract Â· Panda Â·
styled-components Â· Radix Â· Base UI Â· shadcn/ui Â· React Aria Â· MUI Â· Mantine Â· Chakra
...
- **[Emil Kowalski](https://emilkowal.ski)** â€” the motion chapter's whole framework: the frequency test, no `ease-in` on enters, never `scale(0)`, exits faster than enters, origin at the trigger, transitions over keyframes. Author of [Sonner](https://github.com/emilkowalski/sonner), [Vaul](https://github.com/emilkowalski/vaul) and [animations.dev](https://animations.dev). Go there for the depth.
- **[Paul Bakaus](https://github.com/pbakaus/impeccable)** (`impeccable`) and **Anthropic** (`frontend-design`) â€” the router-plus-commands architecture and audit-first posture.
- **[Leon Lin](https://github.com/Leonxlnx/taste-skill)** (`taste-skill`) â€” the anti-slop catalogue converges with this work.
- **Apple** â€” WWDC 2018 session 803, *Designing Fluid Interfaces*, for the velocity and interruptibility model.
- **[BjÃ¶rn Ottosson](https://bottosson.github.io/posts/oklab/)** â€” the OKLab/OKLCH maths in `scripts/lib/color.mjs`.
- **W3C WAI**, **Google Chrome**, **NN/g**, **Refactoring UI**, **Rauno Freiberg**, **Josh Comeau** â€” the standards and writing the reference chapters lean on.
...
- False positives from the scanners â€” these are the most valuable reports
...
(GitHub OIDC) â€” no `NPM_TOKEN` is stored anywhere.
...
**MIT** Â© [shadowfull12](https://github.com/ShadowFull12)
